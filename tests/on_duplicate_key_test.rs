...
/// Test INSERT ... ON DUPLICATE KEY UPDATE triggers on unique index conflict (not just PK)
#[test]
fn test_on_duplicate_key_unique_index_conflict() {
    let storage = Arc::new(RwLock::new(MemoryStorage::new()));
    let mut engine = ExecutionEngine::new(storage);

    engine
        .execute("CREATE TABLE t1 (id INTEGER PRIMARY KEY, email TEXT)")
        .unwrap();

    // Create unique index on email column
    engine
        .execute("CREATE UNIQUE INDEX idx_email ON t1 (email)")
        .unwrap();

    // Insert first row
    engine
        .execute("INSERT INTO t1 VALUES (1, 'alice@test.com')")
        .unwrap();

    // Insert with same email but different id - should trigger ON DUPLICATE KEY UPDATE
    engine
        .execute("INSERT INTO t1 VALUES (2, 'alice@test.com') ON DUPLICATE KEY UPDATE id=999")
        .unwrap();

    let result = engine.execute("SELECT * FROM t1").unwrap();
    // Should have only 1 row (updated, not inserted)
    assert_eq!(result.rows.len(), 1);
    // The id should be updated to 999
    assert_eq!(result.rows[0][0], sqlrustgo_types::Value::Integer(999));
    assert_eq!(
        result.rows[0][1],
        sqlrustgo_types::Value::Text("alice@test.com".to_string())
    );
}
