...
fn test_merge_basic_syntax() {
    ...
    engine
        .execute("CREATE TABLE target (id INTEGER, value TEXT)")
        .unwrap();
    engine
    ...
    engine
        ...
        .unwrap();
    engine
        .execute("INSERT INTO source VALUES (1, 'updated')")
        .unwrap();
    ...
    assert!(
        result.is_ok(),
        "MERGE should parse and execute: {:?}",
        result
    );
}
...
fn test_merge_when_matched() {
    ...
    assert!(
        result.is_ok(),
        "MERGE WHEN MATCHED should work: {:?}",
        result
    );
}
...
fn test_merge_when_not_matched() {
    ...
    assert!(
        result.is_ok(),
        "MERGE WHEN NOT MATCHED should work: {:?}",
        result
    );
}
