...
fn get_query(q: &str) -> Option<&'static str> {
    match q {
        ...
        "Q3"  => Some("SELECT orders.o_orderkey, SUM(lineitem.l_extendedprice) FROM orders JOIN lineitem ON orders.o_orderkey = lineitem.l_orderkey WHERE orders.o_orderdate < '1995-03-15' GROUP BY o_orderkey"),
        ...
        "Q10" => Some("SELECT customer.c_custkey, SUM(lineitem.l_extendedprice) FROM customer JOIN orders ON customer.c_custkey = orders.o_custkey JOIN lineitem ON orders.o_orderkey = lineitem.l_orderkey WHERE orders.o_orderdate >= '1993-10-01' GROUP BY c_custkey"),
