...
fn test_fts_basic_syntax() {
    ...
    engine
        .execute("CREATE TABLE articles (id INTEGER, title TEXT, content TEXT)")
        .unwrap();

    engine
        .execute("INSERT INTO articles VALUES (1, 'Hello World', 'This is a test article')")
        .unwrap();
    ...
    let result =
        engine.execute("SELECT * FROM articles WHERE MATCH(title, content) AGAINST('rust')");
...
fn test_fts_empty_table() {
    let mut engine = create_engine();
    engine
        .execute("CREATE TABLE empty_fts (id INTEGER, text TEXT)")
        .unwrap();

    let result = engine.execute("SELECT * FROM empty_fts WHERE MATCH(text) AGAINST('nothing')");
    assert!(
        result.is_ok(),
        "FTS on empty table should work: {:?}",
        result
    );
}
...
fn test_fts_no_matching_documents() {
    let mut engine = create_engine();
    engine
        .execute("CREATE TABLE fts_test (id INTEGER, content TEXT)")
        .unwrap();
    engine
        .execute("INSERT INTO fts_test VALUES (1, 'Hello World')")
        .unwrap();
    engine
        .execute("INSERT INTO fts_test VALUES (2, 'Goodbye World')")
        .unwrap();
...
fn test_fts_search_returns_correct_results() {
    ...
    engine
        .execute(
            "INSERT INTO articles VALUES (2, 'Database Basics', 'Introduction to SQL databases')",
        )
        .unwrap();
    engine
        .execute(
            "INSERT INTO articles VALUES (3, 'Web Development', 'Building web apps with Rust')",
        )
        .unwrap();

    // Search for 'rust' - should return rows 1 and 3
    let result =
        engine.execute("SELECT id, title FROM articles WHERE MATCH(title, body) AGAINST('rust')");
...
fn test_fts_case_insensitive_search() {
    ...
    engine
        ...
        .unwrap();
    engine
        .execute("INSERT INTO docs VALUES (2, 'HELLO WORLD')")
        .unwrap();
    engine
        .execute("INSERT INTO docs VALUES (3, 'hello world')")
        .unwrap();
    ...
    assert_eq!(
        result.rows.len(),
        3,
        "Case-insensitive search should find all 3 rows"
    );
}
...
fn test_fts_empty_search_term() {
    let mut engine = create_engine();
    engine
        .execute("CREATE TABLE test (id INTEGER, content TEXT)")
        .unwrap();
    engine
        .execute("INSERT INTO test VALUES (1, 'Hello World')")
        .unwrap();
...
fn test_fts_dml_auto_sync() {
    ...
    engine
        .execute("INSERT INTO articles VALUES (1, 'Rust Programming', 'Learn Rust')")
        .unwrap();
    engine
        .execute("INSERT INTO articles VALUES (2, 'Database Basics', 'Introduction to SQL')")
        .unwrap();

    // Search for 'rust' - should find row 1
    let result = engine
        .execute("SELECT id FROM articles WHERE MATCH(title, body) AGAINST('rust')")
        .unwrap();
    ...
    engine
        .execute("UPDATE articles SET title = 'Rust Deep Dive' WHERE id = 1")
        .unwrap();

    // Row 1 should still be findable by new title
    let result = engine
        .execute("SELECT id FROM articles WHERE MATCH(title, body) AGAINST('deep')")
        .unwrap();
    assert_eq!(
        result.rows.len(),
        1,
        "Should find 1 row with 'deep' after UPDATE"
    );
    ...
    assert_eq!(
        result.rows.len(),
        1,
        "Should still find row 1 by 'rust' in body"
    );
...
fn test_fts_select_list_expression() {
    ...
    let result = engine
        .execute("SELECT id, MATCH(title, body) AGAINST('rust') AS relevance FROM articles")
        .unwrap();
    assert_eq!(result.rows.len(), 3, "Should return all 3 rows");

    // Rows containing 'rust' should have relevance = true (1)
    let rust_relevance: Vec<bool> = result
        .rows
        .iter()
        .map(|row| {
    ...
    assert_eq!(
        rust_relevance,
        vec![true, false, true],
        "id=1 and id=3 should match 'rust'"
    );

    // Test FTS in SELECT with WHERE filter
    let result = engine
        .execute(
            "SELECT id, MATCH(title) AGAINST('database') AS title_match FROM articles WHERE id > 1",
        )
        .unwrap();
    assert_eq!(result.rows.len(), 2, "Should return 2 rows");
    let title_match: Vec<bool> = result
        .rows
        .iter()
        .map(|row| {
            if let sqlrustgo_types::Value::Boolean(b) = &row[1] {
    ...
    assert_eq!(
        title_match,
        vec![true, false],
        "id=2 title_match=true, id=3 title_match=false"
    );
}
