    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CorrectionError::ChainBroken {
                ...
                write!(
                    f,
                    "Correction chain broken for {}: expected {}, actual {}",
...
impl CorrectionReason {
    pub fn new(
        ...
        if explanation.trim().is_empty() {
            return Err(CorrectionError::InvalidReason {
...
impl RecordCorrectionChain {
    ...
    pub fn get_correction(&self, correction_id: &str) -> Option<&CorrectionEntry> {
        self.corrections
            .iter()
            .find(|e| e.correction_id == correction_id)
    }
...
impl CorrectionChain {
    ...
    pub fn get_or_create_chain(
        ...
        record_id: &str,
        table_name: &str,
    ) -> &mut RecordCorrectionChain {
        let key = format!("{}:{}", table_name, record_id);
        if !self.chains.contains_key(&key) {
            self.chains.insert(
                key.clone(),
                RecordCorrectionChain::new(record_id.to_string(), table_name.to_string()),
            );
        }
        ...
        for (key, chain) in &self.chains {
            for correction in chain.get_corrections() {
                if !correction.verify_checksum() {
                    return Err(CorrectionError::SignatureInvalid {
                        reason: format!(
                            "Checksum failed for correction {} in chain {}",
                            correction.correction_id, key
                        ),
                    });
                }
            }
...
mod tests {
    ...
    fn test_correction_reason_validation() {
        ...
        assert!(matches!(
            invalid,
            Err(CorrectionError::InvalidReason { .. })
        ));
    }
    ...
    fn test_correction_entry_checksum() {
        let reason =
            CorrectionReason::new("CLARITY", "Initial entry unclear", "operator-1").unwrap();
    ...
    fn test_correction_chain_add_corrections() {
        let mut chain =
            RecordCorrectionChain::new("record-1".to_string(), "batch_records".to_string());

        let reason1 =
            CorrectionReason::new("DATA_ENTRY_ERROR", "Initial typo", "operator-1").unwrap();
        ...
        let reason2 =
            CorrectionReason::new("CLARITY", "Needed more precision", "operator-2").unwrap();
