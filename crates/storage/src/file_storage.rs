...
use crate::engine::{TableData, TableInfo};
...
use crate::engine::{Record, StorageEngine};

impl StorageEngine for FileStorage {
    fn scan(&self, table: &str) -> sqlrustgo_types::SqlResult<Vec<Record>> {
        Ok(self
            .get_table(table)
    ...
    fn insert(&mut self, table: &str, records: Vec<Record>) -> sqlrustgo_types::SqlResult<()> {
    ...
    fn delete(&mut self, table: &str, _filters: &[Value]) -> sqlrustgo_types::SqlResult<usize> {
        if let Some(ref mut data) = self.tables.get_mut(table) {
    ...
    ) -> sqlrustgo_types::SqlResult<usize> {
        Ok(self.get_table(table).map(|d| d.rows.len()).unwrap_or(0))
    }

    fn create_table(&mut self, info: &TableInfo) -> sqlrustgo_types::SqlResult<()> {
        let table_data = TableData {
            info: info.clone(),
            rows: Vec::new(),
        };
    ...
    fn drop_table(&mut self, table: &str) -> sqlrustgo_types::SqlResult<()> {
        self.drop_table(table)
            .map_err(|e| sqlrustgo_types::SqlError::ExecutionError(e.to_string()))?;
        Ok(())
    }

    fn get_table_info(&self, table: &str) -> sqlrustgo_types::SqlResult<TableInfo> {
    ...
    fn create_index(&self, _table: &str, _column: &str, _column_index: usize) -> sqlrustgo_types::SqlResult<()> {
        Ok(())
    }

    fn drop_index(&self, _table: &str, _column: &str) -> sqlrustgo_types::SqlResult<()> {
        Ok(())
    }
}
