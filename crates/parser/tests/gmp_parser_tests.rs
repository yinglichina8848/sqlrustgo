...
fn test_parse_register_device_basic() {
    ...
    assert!(result.is_ok(), "Failed to parse REGISTER DEVICE: {:?}", result);
    match result.unwrap() {
...
fn test_parse_register_device_with_fingerprint() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse REGISTER DEVICE with fingerprint: {:?}", result);
    match result.unwrap() {
        Statement::RegisterDevice(_) => {}
        _ => panic!("Expected RegisterDevice statement"),
    }
...
fn test_parse_device_heartbeat() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse DEVICE HEARTBEAT: {:?}", result);
    match result.unwrap() {
...
fn test_parse_device_collect() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse DEVICE COLLECT: {:?}", result);
    match result.unwrap() {
...
fn test_parse_create_sop_with_version_keyword() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse CREATE SOP with VERSION keyword: {:?}", result);
    match result.unwrap() {
        Statement::CreateSOP(_) => {}
        _ => panic!("Expected CreateSOP statement"),
    }
...
fn test_parse_record_training() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse RECORD TRAINING: {:?}", result);
    match result.unwrap() {
...
fn test_parse_register_calibration_device() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse CALIBRATION DEVICE: {:?}", result);
    match result.unwrap() {
...
fn test_parse_register_calibration_device_with_interval() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse CALIBRATION DEVICE with INTERVAL: {:?}", result);
    match result.unwrap() {
        Statement::RegisterCalibrationDevice(_) => {}
        _ => panic!("Expected RegisterCalibrationDevice statement"),
    }
...
fn test_parse_record_calibration_pass() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse RECORD CALIBRATION PASS: {:?}", result);
    match result.unwrap() {
...
fn test_parse_record_calibration_fail() {
    ...
    let result = parse(sql);
    assert!(result.is_ok(), "Failed to parse RECORD CALIBRATION FAIL: {:?}", result);
    match result.unwrap() {
        Statement::RecordCalibration(_) => {}
        _ => panic!("Expected RecordCalibration statement"),
    }
