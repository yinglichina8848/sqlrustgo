...
pub mod expression {
    ...
    /// Filter RecordBatch using boolean array
    pub fn filter_batch(batch: &RecordBatch, filter: &BooleanArray) -> RecordBatch {
        let mut new_columns = Vec::new();
        for col in batch.columns() {
            if let Some(int_arr) = col.as_any().downcast_ref::<IntArray>() {
                new_columns.push(Arc::new(int_arr.filter(filter.values())) as ArrayRef);
            } else {
